...
class func
{
    ...
    {
        /**@var $this_main \Modules\LA\Market\Market*/
        ...
        if ($this->market != false) {
            ...
            $this->datatable = new \DataTable('Modules\\\LA\\\Market\\\Market', 'ajax_get_market_list');
            $this->datatable_column = array(
                ...
                'price' => array(
                    ...
                    'formatter' => function ($val, $row) {
                        ...
                        $r .= '</span><button type="submit" class="btn btn-sm btn-outline-primary submit-btn" ' . btn_ajax("Modules\LA\Market\Market", "ajax_buy_shop_popup", ['id' => $row['id']]) . '>' . get_lang('market.lang')['buy'] . '</button>';
            ...
            $this->datatable_column_character = array(
                ...
                'char_inventory' => array(
                    ...
                    'formatter' => function ($val, $row) {
                        ...
                        return $r . '<button type="submit" class="btn btn-sm btn-outline-primary submit-btn ml-1" ' . btn_ajax("Modules\LA\Market\Market", "ajax_show_inventory", ['id' => $row['shop_id']]) . '>' . get_lang('market.lang')['all_inventory'] . '</button>';
                    }
                ),
                'price' => array(
                    'name' => 'Цена',
    ...
    public function widget_market_info(){
        if($this->market['balance'] !== false) {
            return '';
        }
        ...
        return get_instance()->fenom->fetch(
            get_tpl_file('widget_market_info.tpl', get_class($this->this_main)),
            array_merge(
                array(
                    'market' => $this->market,
                    'payment_system' => get_instance()->config['payment_system'],
    ...
    public function ajax_refresh_info_two(){
            ...
            if ($response['ok']) {
                ...
                if (isset($response['error'])) {
                    ...
                    if (isset($response["response"]->data->user_data)) {
                        ...
                        $send = get_instance()->ajaxmsg->notify((string)$response["response"]->success)->html($this->widget_market_info(), '#w_user_info')->success();
